...
const mockChats = [
  ...
  { id: 'dm2', userId: 'u456', userName: 'ConnectPro Bot', lastMessage: 'Here are some resources based on our chat...', timestamp: new Date(Date.now() - 3600000), unreadCount: 0, avatar: '/ai-avatar.png' }, // Example AI chat
        ...
        <CardContent className="space-y-2">
          {loading ? (
            ...
            mockChats.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).map(chat => (
              <Link key={chat.id} href={`/community/chat/${chat.isGroup ? `group/${chat.groupId}` : `dm/${chat.userId}`}`} passHref>
                <Card className="flex items-center justify-between p-3 hover:bg-muted/50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3 overflow-hidden">
                    <Avatar className="h-10 w-10 shrink-0">
                       {!chat.isGroup && <AvatarImage src={chat.avatar} alt={chat.userName} />}
                      <AvatarFallback>
