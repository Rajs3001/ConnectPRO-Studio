  ...
  return (
    <AppLayout userType="user">
      <div className="container mx-auto py-8">
        <h1 className="text-3xl font-bold mb-6">Welcome, Alice!</h1> {/* TODO: Use dynamic name */}

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 bg-primary text-primary-foreground">
            <CardHeader>
              ...
              {recentChats.length > 0 ? (
                <ul className="space-y-4">
                  {recentChats.map(chat => (
                    <li key={chat.id} className="flex justify-between items-start p-3 border rounded-md hover:bg-muted/50 cursor-pointer"> {/* Make clickable */}
                     <Link href={`/user/chat/${chat.professional === 'AI Counselor' ? 'ai' : chat.id}`} className="w-full">
                       <div className="flex justify-between items-start w-full">
                          ...
                          {chat.unread > 0 && (
                            <span className="bg-primary text-primary-foreground text-xs font-bold rounded-full px-2 py-0.5 ml-2">
                              {chat.unread}
                            </span>
                          )}
                       </div>
